var OctalGlyphHex = (() => {
  ...
  __export(octalGlyphHex_exports, {
    ...
    render: () => render,
    renderSvg: () => renderSvg,
    renderSvgWithFontFile: () => renderSvgWithFontFile,
    splitOctalChunks: () => splitOctalChunks,
    valueToOctalString: () => valueToOctalString
  });
  ...
  function multiPolygonToPath(multiPolygon, precision = 2) {
    return multiPolygon.flatMap(
      (polygon) => polygon.map((ring) => {
        ...
        const lines = rest.map((point) => `L ${formatNumber(point[0], precision)} ${formatNumber(point[1], precision)}`).join(" ");
  ...
  function boundsToViewBox(bounds, padding, precision) {
    ...
    ].map((value) => formatNumber(value, precision)).join(" ");
  }
  ...
  function attributesToString(attributes) {
    return Object.entries(attributes).filter(([, value]) => value != null && value !== false).map(([key, value]) => `${key}="${escapeAttribute(value)}"`).join(" ");
  }
